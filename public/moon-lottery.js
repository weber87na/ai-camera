...
const MIN_DESKTOP_CARDS = 300;
const FALLBACK_CARD_COUNT = window.innerWidth < 700 ? 48 : 72;
const LAYOUTS = ["moon", "cube", "ai", "nkust", "rabbit", "plane"];
const stage = document.querySelector("#moonStage");
const drawButton = document.querySelector("#drawButton");

const scene = new THREE.Scene();
scene.background = new THREE.Color(0x010208);
scene.fog = new THREE.FogExp2(0x010208, 0.017);
...
const cardGeometry = new THREE.BoxGeometry(0.72, 0.91, 0.035);
...
function createPhotoTexture(image) {
    ...
    drawCover(context, image, 8, 8, 240, 308);
    const vignette = context.createRadialGradient(128, 140, 46, 128, 157, 183);
    vignette.addColorStop(0, "rgba(255,255,255,0)");
    vignette.addColorStop(1, "rgba(3,7,16,0.2)");
    context.fillStyle = vignette;
    context.fillRect(8, 8, 240, 308);
...
function textureForEntry(entry) {
    if (!entry.texture) entry.texture = createPhotoTexture(entry.image);
...
function syncCardBaseInstances() {
    if (!cardBaseInstances) return;
    cards.forEach((card, index) => {
        if (card.visible) {
            card.updateMatrix();
            cardBaseInstances.setMatrixAt(index, card.matrix);
        } else {
            cardInstanceMatrix.compose(card.position, card.quaternion, invisibleCardScale);
            cardBaseInstances.setMatrixAt(index, cardInstanceMatrix);
        }
    });
    cardBaseInstances.instanceMatrix.needsUpdate = true;
}
...
function rebuildGallery() {
    ...
    cardBaseInstances = new THREE.InstancedMesh(cardGeometry, edgeMaterial, count);
    cardBaseInstances.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    cardBaseInstances.frustumCulled = false;
    gallery.add(cardBaseInstances);

    for (let index = 0; index < count; index += 1) {
        const entry = entries[index];
        const texture = textureForEntry(entry);
        ...
        const cardFront = new THREE.Mesh(cardFrontGeometry, frontMaterial);
        cardFront.position.z = 0.019;
        card.add(cardFront);
        card.userData.entry = entry;
        card.userData.index = index;
        card.userData.front = cardFront;
        card.userData.photoMaterial = frontMaterial;
...
for (let index = 0; index < 22; index += 1) {
    ...
    const material = new THREE.MeshBasicMaterial({
        map: laserTexture,
        color: index % 4 === 0 ? 0xffffff : 0xb8d4ff,
        transparent: true,
        opacity: 0,
        blending: THREE.AdditiveBlending,
        depthTest: true,
...
function beginLottery(now) {
    ...
    const eligible = todayEntries.length ? todayEntries : imageLibrary;
    winnerEntry = eligible[Math.floor(Math.random() * eligible.length)];

    state = "gathering";
    lotteryPhaseStartedAt = now;
    drawButton.disabled = true;
    drawButton.querySelector("span").textContent = "集結中";
    controls.enabled = false;
    gallery.rotation.set(0, 0, 0);
    gallery.position.set(0, 0, 0);
    resetExplosion();
    updateLunarHalo(0.01);
    updateRadiantLight(0.01, now);
    beginMorph("lottery", now, true);
}

function revealWinner(now) {
    cards.forEach(card => { card.visible = false; });
    const texture = createPhotoTexture(winnerEntry.image);
    ...
    winnerMesh = new THREE.Mesh(cardGeometry, [edgeMaterial, edgeMaterial, edgeMaterial, edgeMaterial, frontMaterial, backMaterial]);
    winnerMesh.position.set(0, -0.3, 2.8);
    winnerMesh.rotation.set(-0.035, 0.04, -0.05);
...
function updateLottery(now, delta) {
    if (state === "gathering") {
        ...
        const charge = THREE.MathUtils.clamp(elapsed / 2.6, 0, 1);
        gallery.rotation.y += delta * (0.16 + charge * 0.26);
        gallery.rotation.x = Math.sin(now * 1.1) * 0.035;
        ...
        if (elapsed >= 2.8) {
            state = "charging";
            lotteryPhaseStartedAt = now;
            drawButton.querySelector("span").textContent = "滿月";
        }
        return;
    }
    ...
    if (state === "charging") {
        const elapsed = now - lotteryPhaseStartedAt;
        const charge = THREE.MathUtils.clamp(elapsed / 1.05, 0, 1);
        gallery.rotation.y += delta * (0.42 + charge * 0.42);
        const pulse = 1 + Math.sin(elapsed * 9) * 0.012 * charge;
        gallery.scale.setScalar(pulse);
        keepLotteryPivotCentered();
        setPhotoCharge(1 + charge * 0.35, now);
        updateLunarHalo(0.96 + charge * 0.16, Math.sin(now * 7.2));
        updateRadiantLight(0.92 + charge * 0.34, now, charge * 0.08);
        if (elapsed >= 1.05) {
            cards.forEach((card, index) => {
    ...
    if (state === "exploding") {
        const progress = THREE.MathUtils.clamp((now - lotteryPhaseStartedAt) / 1.05, 0, 1);
    ...
    if (state === "winner-enter" && winnerMesh) {
        const progress = THREE.MathUtils.clamp((now - lotteryPhaseStartedAt) / 0.9, 0, 1);
        winnerMesh.scale.setScalar(Math.max(0.01, easeOutBack(progress) * 5.2));
        winnerMesh.position.y = THREE.MathUtils.lerp(-1.2, 0.15, easeOutCubic(progress));
        if (progress >= 1) {
            state = "winner";
...
async function init() {
    ...
    await refreshPhotos();
    rebuildGallery();
    lastShapeChangeAt = performance.now() / 1000;
